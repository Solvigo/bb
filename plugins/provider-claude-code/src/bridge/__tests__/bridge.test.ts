...
describe("bridge", () => {
  ...
  // Regression for #2019 / #1712: the runtime never rebuilds a bridge session
  // for an option change, so `/plan` on an already-loaded session only reached
  // the bridge as providerOptions on turn/start. The bridge stripped the
  // mention and pushed the prompt without ever switching the live SDK session
  // into Plan mode, so Claude executed the task instead of planning it.
  it("switches an already-loaded session into Plan mode when a turn carries /plan", async () => {
    const bridge = createBridgeJsonRpcTestHarness(handleLine);
    const queries: ControlledClaudeQuery[] = [];
    queryMock.mockImplementation(() => {
      const query = createControlledClaudeQuery();
    ...
    try {
      const threadId = "thread-plan-live-session";
      await startBridgeThread({ bridge, threadId });
      ...
      // The live session switched modes instead of being rebuilt.
      ...
      // A mid-turn /plan steer while already in Plan mode is a no-op.
      bridge.sendRequest(
        4,
        "turn/steer",
        canonicalTurnParams({
      ...
      // Approving the plan restores the user's preset, as for a session that
